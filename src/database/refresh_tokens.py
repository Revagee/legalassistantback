...
class RefreshToken(BaseEntity):
    ...
    async def create_token(
        cls, user_id: UUID, token_hash: str, session: AsyncSession, expires_in_days: int = 30
    ) -> "RefreshToken":
    ...
    @classmethod
    async def get_by_token_hash(cls, token_hash: str, session: AsyncSession) -> Optional["RefreshToken"]:
        """Get refresh token by hash if it's valid (not expired or revoked)."""
        query = select(cls).where(
            and_(cls.token_hash == token_hash, cls.expires_at > datetime.now(UTC), cls.revoked_at.is_(None))
        )
    ...
    async def revoke_all_user_tokens(cls, user_id: UUID, session: AsyncSession) -> None:
        ...
        query = select(cls).where(and_(cls.user_id == user_id, cls.revoked_at.is_(None)))
        result = await session.execute(query)
