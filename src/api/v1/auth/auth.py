...
@router.post("/register", response_model=schema.RegisterResponse, status_code=status.HTTP_201_CREATED)
async def register(user_data: schema.UserRegisterRequest):
    ...
    async with session:
        ...
        if not auth_service.password_service.validate_password_strength(user_data.password):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=auth_service.get_password_requirements_error()
            )
        ...
        verification_token, expires_at = auth_service.email_token_service.create_verification_token(None)
        ...
        return schema.RegisterResponse(
            user=user_response, message="Registration successful. Please check your email and verify your account."
        )
...
async def login(user_data: schema.UserLoginRequest):
    ...
    async with session:
        # Authenticate user
        user = await auth_service.authenticate_user(user_data.email.strip(), user_data.password, session)
        if not user:
        ...
        # Create tokens
        access_token, refresh_token, expire = await auth_service.create_user_tokens(user, session)
        ...
        return schema.TokenResponse(
            ...
            expires_in=int(timedelta(minutes=AuthConfig.ACCESS_TOKEN_EXPIRE_MINUTES).total_seconds()),
            user=user_response,
        )
...
async def logout(logout_data: schema.LogoutRequest, current_user: User = Depends(get_current_user)):
...
async def forgot_password(request_data: schema.ForgotPasswordRequest):
    ...
    async with session:
        ...
        if not user:
            ...
            return schema.MessageResponse(message="If the email exists, a password reset link has been sent.")

        # Invalidate any existing password reset tokens
        await PasswordReset.invalidate_user_tokens(user.id, session)

        # Generate password reset token
        reset_token, expires_at = auth_service.email_token_service.create_password_reset_token(user.id)
        token_hash = auth_service.token_service.hash_token(reset_token)

        # Store password reset token
        password_reset = await PasswordReset.create_reset_token(user_id=user.id, token_hash=token_hash, session=session)
...
async def reset_password(request_data: schema.ResetPasswordRequest):
    ...
    async with session:
        # Validate password strength
        if not auth_service.password_service.validate_password_strength(request_data.new_password):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=auth_service.get_password_requirements_error()
            )
        ...
        # Update password
        user.password_hash = auth_service.password_service.hash_password(request_data.new_password)
...
async def verify_email(token: str):
    ...
    async with session:
        ...
        if not user:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Your verification token has expired")
...
async def resend_verification_email(request_data: schema.ResendVerificationRequest):
    ...
    async with session:
        ...
        if not user:
            # Don't reveal if email exists or not
            return schema.MessageResponse(message="If the email exists, a verification email has been sent.")
...
async def change_password(request_data: schema.ChangePasswordRequest, current_user: User = Depends(get_current_user)):
    """Change user's password."""
    session = get_session()

    async with session:
        # Verify current password
        if not auth_service.password_service.verify_password(request_data.current_password, current_user.password_hash):
        ...
        # Update password
        current_user.password_hash = auth_service.password_service.hash_password(request_data.new_password)
        await session.commit()
...
@router.post("/cleanup-unverified", response_model=schema.CleanupResponse)
async def cleanup_unverified_accounts():
    """Clean up unverified accounts older than 24 hours."""
    session = get_session()

    async with session:
        deleted_count = await User.cleanup_unverified_accounts(session)

        logger.info(f"Cleaned up {deleted_count} unverified accounts")

        return schema.CleanupResponse(
            message=f"Cleaned up {deleted_count} unverified accounts", deleted_count=deleted_count
        )
